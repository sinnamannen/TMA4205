...
def run_single_method_regs(method, regs,
    ...
    for reg in regs:
        ...
        # ---------------- CG ----------------
        if method == "CG":
            u, v, res, max_iter, elapsed = cg_main("frame10.png", "frame11.png", reg, tol=1.e-8, maxit=2000, from_file=True, sigma=sigma)

        # ---------------- PCG ----------------
        elif method == "PCG":
            u, v, res, max_iter, elapsed = pcg_main(
                "frame10.png", "frame11.png", reg,
                max_level=mg_max_level,
                s1=mg_s1,
                s2=mg_s2,
                tol=1e-8,
